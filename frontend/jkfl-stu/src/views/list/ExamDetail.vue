<template>
  ...
  <a-layout id="ExamDetail">
    ...
    <a-layout>
      ...
      <a-layout>
        <a-layout-content id="questionContent" :style="{ margin: '24px 16px 0', height: '84vh' }">
          <HighlighterBar
          ...
          <div :style="{ padding: '24px', background: '#fff', height: '84vh' }">
            ...
            <div
              ...
              "
            >
              <!-- <MyEditor :inputHtml="inputValue" @saveAnswer="saveEditorAnswer" /> -->
              <div style="border: 1px solid #ccc; margin-top: 10px">
                <!-- 工具栏 -->
                <Toolbar style="border-bottom: 1px solid #ccc" :editor="wEditor.editor" :defaultConfig="wEditor.toolbarConfig" />
                <!-- 编辑器 -->
                <Editor
                  style="height: 300px; overflow-y: hidden"
                  :defaultConfig="wEditor.editorConfig"
                  v-model="inputValue"
                  @onChange="OnInputChange"
                  @onCreated="onWEditorCreated"
                />
              </div>
...
export default {
  ...
  methods: {
    // wangEditor
    onWEditorCreated(editor) {
      this.wEditor.editor = Object.seal(editor)
    },
    initExam() {
      const that = this
  ...
  beforeDestroy() {
    clearInterval(this.remainingTimer)
    if (this.wEditor.editor) this.wEditor.editor.destroy()
  },
}
</script>
