  ...
  handleSubmit(e){
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        //提交题目信息
        axios.post(examport+'/short_answer/shortAnswerQuestion',{
          saqSubject: this.state.subject,
          saqQuestion: this.state.question,
          saqAnalysis: this.state.analysis,
          saqScore: 10,
          saqLevel: this.props.level,
        },{
          headers:{
  ...
  onImportExcel(file){
    ...
    fileReader.onload = event => {
      try {
        ...
        for (const sheet in workbook.Sheets) {
          ...
          if (workbook.Sheets.hasOwnProperty(sheet)) {
            ...
            data.map((_item,_index)=> {
              let temp = {};
              temp['saqSubject'] = _item['知识点'];
              temp['saqQuestion'] = _item['题干'];
              temp['saqAnalysis'] = _item['解析']===''?'无解析':_item['解析'];
              up.push(temp);
            })
        ...
        console.log(up);
        axios.post(examport+'/short_answer/addlist',{
          list: up
        },{
          headers:{
            "Authorization":getJwtToken(),
