...
class QFillIn extends React.Component {
  ...
  handleSubmit(e){
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        //提交题目信息
        axios.post(examport+'/input/inputQuestion',{
          iqSubject: this.state.subject,
          iqQuestion: this.state.question,
          iqAnswer: this.state.answer,
          iqScore: 3,
          iqLevel: this.props.level,
          iqAnalysis: this.state.analysis,
        },{
          headers:{
  ...
  onImportExcel(file){
    ...
    fileReader.onload = event => {
      try {
        ...
        for (const sheet in workbook.Sheets) {
          ...
          if (workbook.Sheets.hasOwnProperty(sheet)) {
            ...
            data.map((_item,_index)=> {
              let temp = {};
              temp['iqSubject'] = _item['知识点'];
              temp['iqQuestion'] = _item['题干'];
              temp['iqAnswer'] = _item['答案'];
              temp['iqAnalysis'] = _item['解析']===''?'无解析':_item['解析'];
              up.push(temp);
            })
        ...
        console.log(up);
        axios.post(examport+'/input/addlist',{
          list: up
        },{
          headers:{
            "Authorization":getJwtToken(),
      ...
      <div>
        ...
        <Form onSubmit={this.handleSubmit.bind(this)}>
          ...
          >
            <Row>
              <Col span={24}>
                <Input onChange={this.bindinput.bind(this)} rows={4} />
              </Col>
            </Row>
          </FormItem>
          <FormItem
