...
class ChooseCard extends React.Component {
  ...
  componentDidMount() {
    console.log(this.props.singleQuestion.score)
  }
  ...
  render(){
    ...
    const {score} = this.props.singleQuestion;
    ...
    if(this.props.knowledgePointInfo.length !== 0 ) {
      knowledgePointInfo = this.props.knowledgePointInfo.map((item)=>{
        return (
          <Option value={item.pointName} key={item.pointId}>{item.pointName}</Option>
        )
      })
    }
          ...
          <Col span={10}>
            ...
            >
              {getFieldDecorator('score',{
                ...
                <InputNumber defaultValue={score} disabled min={score} onChange={this.scoreChange.bind(this)}/>
              )}
            </FormItem>
          </Col>
        </Row>
