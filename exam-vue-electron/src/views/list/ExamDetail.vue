      ...
      <a-layout :style="{ marginLeft: '230px' }">
        <a-layout-content :style="{ margin: '24px 16px 0',height: '84vh', overflow: 'initial' }">
          <div :style="{ padding: '24px', background: '#fff',height: '84vh'}">
            ...
            <!-- 填空和简答和程序题 -->
            <div v-if="currentQuestion.type === '填空题' || currentQuestion.type === '简答题' || currentQuestion.type === '程序题'">
...
export default {
  ...
  mounted () {
    ...
    getSubDetail(this.$route.params.id)
      .then(res => {
        if (res.code === 0) {
          // 赋值考试对象
          console.log(res)
  ...
  methods: {
    ...
    finishExam () {
      ...
      finishExam(this.$route.params.id, this._mapToJson(this.answersMap))
        .then(res => {
          if (res.code === 0) {
            finishSub(res.data.examRecordId, this._mapToJson(this.subMap))
              .then(res => {
                if (res.code === 0) {
