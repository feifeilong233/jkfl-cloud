...
teach-server： 教育模块是后续新增加的模块，旨在为用户提供随时随地的学习体验，集成在学生端中实现阅读教育功能。

video-server:  教育模块的附属，集成课程会议、视频等一些列常规教育功能。

wx-server:  对接uni-app微信小程序的后端模块，集成课程会议、阅读、视频等一些列常规教育功能。
